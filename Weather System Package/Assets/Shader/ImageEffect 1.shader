    ...
    SubShader
    {
        ...
        Pass
        {
           ...
           /* float fbm(float3 p, const int octaves)
            ...
            /*float density(const float3 p)
            {
                float3 q = p;
                q += float3(0.0, 0.10, 1.0) * time;
            ...
            //float3 lighting(const float3 pos, const float cloudDensity, const float3 backgroundColor, const float pathLength)
            //{
            //    sunDir = normalize(float3(sin(time), 0.0, cos(time)));

            //    float densityLightDir = density(pos + 0.3 * sunDir); // sample in light dir
            ...
            fixed4 raymarching(float3 rayOrigin, float3 rayDir/*, const float3 backgroundCol*/)
            {
                ...
                for (int i = 0; i < STEPS; ++i)
                {
                    ...
                    //float cloudDensity = density(pos);
